...
export default function App() {
  ...
  const handleVerifyChallenge = () => {
    const check = currentChallenge.successCondition(virtualFS, variables);
    if (check.success) {
  ...
  const handleSkipLevel = (lvlId: number) => {
  ...
  const executeTerminalCommand = (line: string) => {
    ...
    switch (cmd.toLowerCase()) {
      ...
      case "chmod":
        if (args.length < 2) {
          setTerminalLines(prev => [...prev, { id: `${Date.now()}-chmod-err`, text: "Uso: chmod <modo-octal> <arquivo>  (ex: chmod 600 backup_credenciais.db)", type: "error", timestamp }]);
          break;
        }
        const permMode = parseInt(args[0], 10);
